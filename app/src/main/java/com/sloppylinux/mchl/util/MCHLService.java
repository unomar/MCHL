...
//    Call<TeamTable> getTeamStats(@Path(value = "listId") long listId);
    ...
    @GET("lists")
    Call<List<TeamTable>> getTeamStats(@Query("seasons") Long seasonId, @Query("leagues") Long leagueId, @Query(value="search", encoded = true) String teamName );

    @GET("events?order=asc")
    Call<List<Event>> listAllEvents();

    @GET("events?order=desc&per_page=100")
    Call<List<Event>> listTeamResults(@Query(value= "search", encoded = true) String teamName, @Query("seasons") Long seasonId, @Query("leagues") Long leagueId, @Query(value="before", encoded = true) String date);

    @GET("events?order=asc&per_page=100")
    Call<List<Event>> listTeamSchedule(@Query(value="search", encoded = true) String teamName, @Query("seasons") Long seasonId, @Query("leagues") Long leagueId, @Query(value="after", encoded = true) String date);
